...
def upConvolution(inp_layer, filter_size, filter_in, filter_out, rectifier=leakyRelu, bias=0.3, scale=2):
    assert len(inp_layer.get_shape()) == 4
    ...
    c1 = rectifier(tf.nn.conv2d(unpooled, w1, [1, 1, 1, 1], 'SAME') + b1)
    #w2 = tf.get_variable('w2', shape=[2, 2, filter_out, filter_out],
    #                     initializer=tf.contrib.layers.xavier_initializer())
    #b2 = tf.get_variable('b2', shape=[filter_out], initializer=tf.constant_initializer(0.3))
...
def downConvolution(inp_layer, filter_size, pool_size, filter_in, filter_out, conv_stride=1, pool_stride=None, pool=True):
    ...
    #w2 = tf.get_variable('w2', shape=[2, 2, filter_out, filter_out],
    #                     initializer=tf.contrib.layers.xavier_initializer())
    #b2 = tf.get_variable('b2', shape=[filter_out], initializer=tf.constant_initializer(0.5))
    #p2 = tf.nn.relu(tf.nn.conv2d(p1, w2, [1, 1, 1, 1], 'SAME') + b2)
    return p1
...
def grid_layer(frame, batch_size, as_image=False):
    ...
    #with tf.variable_scope('c3'):
    #    w = tf.get_variable('w', [2, 2, 3, 1], initializer=tf.contrib.layers.xavier_initializer())
